...
class WebService {
    ...
    async connectPlayer(sock,game,user){
        ...
        await sock.send({action: "initResp",map:await game.data.map.getMapData(),players:(await game.getPlayerTags(getStatus=true)).toList(),countries:await game.data.getCountries()});
